...
ggplot(buscas, aes(x=session_length)) + geom_histogram()
...
A tabela acima é bem interessante, pois ela mostra que a maioria das sessões com até 30 segundos de duração são classificadas como grupo "B". Ao passo que todas as outras sessões com mais tempo de duração são em maior parte do grupo "A".  
...
O gráfico acima mostra que a quantidade de sessões com menos de 30s de duração foram as sessões que apresentaram uma maior quantidade de buscas no grupo "B" em todos os dias. O mesmo não se reflete no grupo "A" que apresenta u  
...
# 
ggplot(temp , aes(x=amount_clicks, y=amount_of_searches, color=session_length_hour)) + facet_wrap( ~group) + geom_point()
